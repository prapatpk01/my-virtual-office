"""Unified launcher — pick the trading system with the MODE env var.
    ...
    MODE=hma     -> HMA Expert MTF V4.3 Sentinel S/R Entry
...
def main() -> None:
    ...
    if mode in ("dual", "dual_entry", "v14", "v1.4"):
        ...
        argv = [sys.executable, "main_v13.py"]
        name = "HMA Expert MTF V4.3 (Sentinel S/R Entry + FX 24/5)"
        if not os.path.isfile(os.path.join(target_dir, "main_v13.py")):
            print(f"FATAL: hma_bot/main_v13.py not found under {HERE}", flush=True)
            sys.exit(1)
    else:
