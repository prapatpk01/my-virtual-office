"""Unified launcher — pick the trading system with the MODE env var.
    ...
    MODE=hma     -> HMA Expert MTF V3.7 (4H→1H→15M→5M + FX 24/5)
...
def main() -> None:
    ...
    if mode in ("dual", "dual_entry", "v14", "v1.4"):
        ...
        argv = [sys.executable, "main_v9.py"]
        name = "HMA Expert MTF V3.7 (FX 24/5 entry gate)"
        if not os.path.isfile(os.path.join(target_dir, "main_v9.py")):
            print(f"FATAL: hma_bot/main_v9.py not found under {HERE}", flush=True)
            sys.exit(1)
    else:
