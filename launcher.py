"""Unified launcher — pick the trading system with the MODE env var.
    ...
    MODE=hma     -> HMA Expert MTF V4.1 Adaptive Sentinel
...
def main() -> None:
    ...
    if mode in ("dual", "dual_entry", "v14", "v1.4"):
        ...
        argv = [sys.executable, "main_v11.py"]
        name = "HMA Expert MTF V4.1 (Adaptive Sentinel + FX 24/5)"
        if not os.path.isfile(os.path.join(target_dir, "main_v11.py")):
            print(f"FATAL: hma_bot/main_v11.py not found under {HERE}", flush=True)
            sys.exit(1)
    else:
