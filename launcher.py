"""Unified launcher — pick the trading system with the MODE env var.
    ...
    MODE=hma     -> HMA Expert MTF V3.3 (4H→1H→15M→5M)
...
def main() -> None:
    ...
    if mode in ("dual", "dual_entry", "v14", "v1.4"):
        ...
        argv = [sys.executable, "main_v5.py"]
        name = "HMA Expert MTF V3.3 (chart + directional alerts)"
        if not os.path.isfile(os.path.join(target_dir, "main_v5.py")):
            print(f"FATAL: hma_bot/main_v5.py not found under {HERE}", flush=True)
            sys.exit(1)
    else:
