"""Unified launcher — pick the trading system with the MODE env var.
    ...
    MODE=hma     -> HMA Expert MTF V4.0 Sentinel (4H→1H→15M→5M + FX 24/5)
...
def main() -> None:
    ...
    if mode in ("dual", "dual_entry", "v14", "v1.4"):
        ...
        argv = [sys.executable, "main_v10.py"]
        name = "HMA Expert MTF V4.0 (Sentinel location-aware + FX 24/5)"
        if not os.path.isfile(os.path.join(target_dir, "main_v10.py")):
            print(f"FATAL: hma_bot/main_v10.py not found under {HERE}", flush=True)
            sys.exit(1)
    else:
