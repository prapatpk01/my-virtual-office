...
Entry logic (3-layer multi-timeframe):
  BUY  (long)  : 4H up + 1H up + price at EMA20(1H) pullback + 15m conditions
  SELL (short) : 4H down + 1H down + price at EMA20(1H) resistance + 15m conditions

15m entry requires min_entry_cond / 4 (default 3/4):
  1. close > EMA9  (micro momentum positive / negative)
  2. close > EMA20 (above / below local support)
  3. RSI(14) in rsi_min-rsi_max range
...
class ScalpTrendStrategy(BaseStrategy):
    ...
    def __init__(self, symbol: str, params: dict = None):
        ...
        self.rsi_min_buy    = self.params.get("rsi_min",        38.0)   # ↓ from 42 — more signals
        self.rsi_max_buy    = self.params.get("rsi_max",        72.0)   # ↑ from 65 — more signals
        self.rsi_min_sell   = self.params.get("rsi_min_sell",   28.0)   # ↓ from 35
        self.rsi_max_sell   = self.params.get("rsi_max_sell",   65.0)   # ↑ from 58
        self.pullback_pct   = self.params.get("pullback_pct",  0.025)   # ↑ from 0.015 — wider zone
        self.vol_period     = self.params.get("vol_period",      20)
        self.vol_mult       = self.params.get("vol_mult",        1.0)   # ↓ from 1.2 — lenient
        self.atr_period     = self.params.get("atr_period",      14)
        self.sl_mult        = self.params.get("sl_mult",         1.5)   # ↑ from 0.8 — ATR×1.5
        self.tp_mult        = self.params.get("tp_mult",        1.875)  # SL×1.25 → R:R 1:1.25
        self.sl_min_pct     = self.params.get("sl_min_pct",    0.010)   # 1.0% min SL
        self.sl_max_pct     = self.params.get("sl_max_pct",    0.060)   # 6.0% max SL
        self.min_entry_cond = self.params.get("min_entry_cond",   3)    # 3/4 conditions required
    ...
    async def analyze(self, candles: list, current_price: float,
        ...
        # ── 1H pullback / resistance zone (last fully closed 1H bar = [-2]) ─
        ...
        wick_bounce_l     = (last_1h.low  <= ema20_1h * 1.005) and (last_1h.close > ema20_1h)
        wick_reject_s     = (last_1h.high >= ema20_1h * 0.995) and (last_1h.close < ema20_1h)
        at_pullback_long  = near_ema or wick_bounce_l
        at_pullback_short = near_ema or wick_reject_s

        # ── 15m indicators (last fully closed bar = [-2]) ─────────────────────
        ...
        vol_ok = (volma_b > 0 and vol_b >= volma_b * self.vol_mult) if self.vol_mult > 0 else True
        ...
        # ── BUY (long): 4H up + 1H up + pullback → 3/4 entry conditions ─────
        if macro_up and mid_up and at_pullback_long:
            c1 = close_b > ema9_b
            c2 = close_b > ema20_b
            c3 = self.rsi_min_buy <= rsi_b <= self.rsi_max_buy
            c4 = vol_ok
            if sum([c1, c2, c3, c4]) >= self.min_entry_cond:
                sl, tp = _sl_tp("long")
                return Signal(
                    SignalType.BUY, self.symbol, current_price,
                    amount=0.08,
                    reason=(f"[ScalpTrend] 4H↑ 1H↑ EMA20pull "
                            f"cond={sum([c1,c2,c3,c4])}/4 RSI={rsi_b:.0f} "
                            f"ATR={atr_v:.0f} RR=1:{rr:.2f}"),
                    confidence=0.65 + sum([c1, c2, c3, c4]) * 0.02,
                    metadata={"stop_loss": sl, "take_profit": tp, "atr": atr_v},
                )

        # ── SELL (short): 4H down + 1H down + resistance → 3/4 entry cond ───
        if macro_down and mid_down and at_pullback_short:
            ...
            if sum([c1, c2, c3, c4]) >= self.min_entry_cond:
                ...
                return Signal(
                    ...
                    amount=0.08,
                    reason=(f"[ScalpTrend] 4H↓ 1H↓ EMA20resist "
                            f"cond={sum([c1,c2,c3,c4])}/4 RSI={rsi_b:.0f} "
                            f"ATR={atr_v:.0f} RR=1:{rr:.2f}"),
                    confidence=0.65 + sum([c1, c2, c3, c4]) * 0.02,
                    metadata={"stop_loss": sl, "take_profit": tp, "atr": atr_v},
