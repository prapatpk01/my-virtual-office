...
class SentinelStrategy(BaseStrategy):
    ...
    def _adaptive_sr(self, candles_15m: list, mtf: dict, price: float, atr: float) -> dict:
        ...
        if s1 is not None and s2 is not None and abs(s1-s2) <= atr*self.sr_merge_atr:
            s1 = (s1+s2)/2.0
        return {"s1": s1, "s2": s2_raw, "r1": r1, "r2": r2_raw, "s1_raw": s1_raw, "r1_raw": r1_raw}
    ...
    def _mcdx_context(self, candles: list, mtf: dict) -> dict:
        ...
        look = 50
        lo50 = float(np.min(lows[-look:])); hi50 = float(np.max(highs[-look:])); rng = max(hi50-lo50, 1e-12)
        ...
        smart_hist=[]; retail_hist=[]
        for i in range(n-6, n):
            j0=max(0,i-look+1); l=float(np.min(lows[j0:i+1])); h=float(np.max(highs[j0:i+1])); d=max(h-l,1e-12)
            sm=self._clamp(((closes[i]*0.96)-l)/d*100.0,0,100); ft=self._clamp(((closes[i]*1.04)-l)/d*100.0,0,100)
        ...
        smart_rising=smart>smart_sma and smart>smart_hist[-2]; smart_falling=smart<smart_sma and smart<smart_hist[-2]
        retail_falling=retail<retail_sma and retail<retail_hist[-2]; retail_rising=retail>retail_sma and retail>retail_hist[-2]
        signed=np.sign(np.diff(closes, prepend=closes[0]))*vols; obv=np.cumsum(signed); obv20=obv[-20:]; obv_std=float(np.std(obv20))
        obv_z=0.0 if obv_std<=1e-12 else (float(obv[-1])-float(np.mean(obv20)))/obv_std
        obv_norm=self._clamp((obv_z+3.0)/6.0*100.0,0,100)
        pv=closes[-80:]*vols[-80:]; vwap=float(np.sum(pv)/max(float(np.sum(vols[-80:])),1e-12)); dev=float(np.std(closes[-20:]-vwap))
        vwap_score=self._clamp(50+(closes[-1]-vwap)/max(dev,1e-9)*20,0,100)
        upvol=float(np.sum(vols[-20:][np.diff(closes[-21:])>0])) if n>=21 else 0.0; dnvol=float(np.sum(vols[-20:][np.diff(closes[-21:])<0])) if n>=21 else 0.0
        vol_agree=50.0 if upvol+dnvol<=0 else 100.0*upvol/(upvol+dnvol); volume_flow=0.50*vol_agree+0.30*obv_norm+0.20*vwap_score
        rsi_arr=self.rsi(list(closes),14); _,_,hist=self.macd(list(closes),12,26,9); rsi=float(rsi_arr[-1]) if np.isfinite(rsi_arr[-1]) else 50.0
        hh=hist[-100:][np.isfinite(hist[-100:])]; macd_score=50.0 if len(hh)==0 else self._norm(float(hist[-1]),float(np.min(hh)),float(np.max(hh)))
        momentum=0.45*rsi+0.55*macd_score
        e20=self.ema(list(closes),20); e50=self.ema(list(closes),50); adx_arr,pdi,mdi=self.adx(candles,14); adx=float(adx_arr[-1]) if np.isfinite(adx_arr[-1]) else 0.0
        bull_trend=e20[-1]>e50[-1] and e20[-1]>e20[-4]; bear_trend=e20[-1]<e50[-1] and e20[-1]<e20[-4]; trend_dir=75.0 if bull_trend else 25.0 if bear_trend else 50.0
        dmi_score=self._clamp(50+adx*0.8,50,100) if pdi[-1]>mdi[-1] else self._clamp(50-adx*0.8,0,50) if mdi[-1]>pdi[-1] else 50.0; trend_score=0.55*trend_dir+0.45*dmi_score
        h1=list((mtf or {}).get("1h") or []); mtf_score=50.0; htf_bull=htf_bear=False
        if len(h1)>=55:
        ...
        flow_bull=smart_flow>=55; flow_bear=smart_flow<=45; structure=self._structure(candles); struct_bias=1 if structure=="BULL" else -1 if structure=="BEAR" else 0
        ...
        return {"long_score":round(long_score,1),"short_score":round(short_score,1),"smart_money":round(smart,1),"smart_flow":round(smart_flow,1),"flow_bull":flow_bull,"flow_bear":flow_bear,"smart_rising":smart_rising,"smart_falling":smart_falling,"retail_falling":retail_falling,"retail_rising":retail_rising,"structure":structure,"adx":round(adx,1),"ready":True}
    ...
    def _sentinel_context(self, candles: list, mtf: dict) -> dict:
        ...
        e20=self.ema(closes,20); e50=self.ema(closes,50); hma16=self.hma(closes,16); structure=self._structure(candles)
        bull=bool(e20[-1]>=e50[-1] and e20[-1]>e20[-4] and closes[-1]>=e20[-1]); bear=bool(e20[-1]<=e50[-1] and e20[-1]<e20[-4] and closes[-1]<=e20[-1])
        _,_,hist=self.macd(closes,12,26,9); r=self.rsi(closes,14); fast=float(hma16[-1]-hma16[-3]) if np.isfinite(hma16[-1]) and np.isfinite(hma16[-3]) else 0.0
        histv=float(hist[-1]) if np.isfinite(hist[-1]) else 0.0; rsiv=float(r[-1]) if np.isfinite(r[-1]) else 50.0
        sme_bull=fast>0 and histv>=0 and rsiv>=45; sme_bear=fast<0 and histv<=0 and rsiv<=55; bias="BULL" if bull else "BEAR" if bear else "BALANCED"
    ...
    async def analyze(self, candles: list, current_price: float, mtf_candles: dict = None) -> Signal:
        ...
        atr_arr=self.atr(candles,14); atr=float(atr_arr[-1]) if np.isfinite(atr_arr[-1]) else 0.0
        if atr<=0:
        ...
        bar=candles[-1]; close=float(bar.close); op=float(bar.open); low=float(bar.low); high=float(bar.high)
        sr=self._adaptive_sr(candles,mtf_candles or {},close,atr); s1,s2,r1,r2=sr["s1"],sr["s2"],sr["r1"],sr["r2"]
        sx=self._sentinel_context(candles,mtf_candles or {}); mc=self._mcdx_context(candles,mtf_candles or {})
        ...
        location_atr=(r1-s1)/atr; meta["location_atr"]=round(location_atr,2)
        ...
        zone=self.entry_zone_atr*atr; long_at_s1=(low <= s1+zone and close >= s1 and close>op); short_at_r1=(high >= r1-zone and close <= r1 and close<op)
        long_sl=s2-self.sl_buffer_atr*atr; long_tp=r1; short_sl=r2+self.sl_buffer_atr*atr; short_tp=s1
        long_risk=close-long_sl; long_reward=long_tp-close; short_risk=short_sl-close; short_reward=close-short_tp
        long_rr=(long_reward/long_risk) if long_risk>0 else 0.0; short_rr=(short_reward/short_risk) if short_risk>0 else 0.0
