"""Sentinel V1.11 — hybrid 1H S/R + fast 15M execution + dynamic proximity.

Core rules
- 1H RAW OHLC is the primary S1/S2/R1/R2 map and ATR14 source.
- 1H Heiken Ashi pivots are a fallback only when RAW OHLC has no valid level on that side.
- 15M RAW OHLC ONLY executes proximity rejection / reclaim / displacement.
- LONG requires only 1H S1. S2 is preferred but is NOT required.
  ...
  must pass; if R1 does not exist after RAW+HA fallback, LONG is OPEN_SKY.
- SHORT requires only 1H R1. R2 is preferred but is NOT required.
  ...
  must pass; if S1 does not exist after RAW+HA fallback, SHORT is OPEN_FLOOR.
...
- OPEN_SKY/OPEN_FLOOR use MCDX-directional dynamic proximity: base 0.60 ATR,
  continuously expanding with score, dominance gap and flow, capped at 1.50 ATR.
...
class SentinelStrategy(BaseStrategy):
    ...
    def __init__(
        ...
        open_ended_max_proximity_atr: float = 1.50,
        ...
        self.open_ended_proximity_atr = max(self.reversal_proximity_atr, float(open_ended_proximity_atr))
        self.open_ended_max_proximity_atr = max(
            self.open_ended_proximity_atr,
            min(1.50, float(open_ended_max_proximity_atr)),
        )
    ...
    @staticmethod
    def _heikin_ashi_pivots(candles: list, span: int) -> tuple[list[float], list[float]]:
        """Return confirmed pivot highs/lows from synthetic 1H Heiken Ashi bars.

        HA is used only for finding a fallback map level. Execution and ATR remain RAW OHLC.
        """
        if len(candles) < span * 2 + 3:
            return [], []
        ...
        for i, c in enumerate(candles):
            ...
            if i == 0 or prev_ha_open is None or prev_ha_close is None:
                ha_open = (op + cl) / 2.0
            else:
        ...
        for i in range(span, len(ha_highs) - span):
            hi = ha_highs[i]
            lo = ha_lows[i]
            if hi >= max(ha_highs[i-span:i]) and hi >= max(ha_highs[i+1:i+span+1]):
                pivot_highs.append(float(hi))
            if lo <= min(ha_lows[i-span:i]) and lo <= min(ha_lows[i+1:i+span+1]):
    ...
    def _sr_map_1h(self, mtf: dict, price: float) -> dict:
        ...
        raw_h, raw_l = self._confirmed_pivots(h1[-180:], self.pivot_span)
        ha_h, ha_l = self._heikin_ashi_pivots(h1[-180:], self.pivot_span)

        raw_res = sorted({float(x) for x in raw_h if float(x) > price})
        raw_sup = sorted({float(x) for x in raw_l if float(x) < price}, reverse=True)
        ha_res = sorted({float(x) for x in ha_h if float(x) > price})
        ha_sup = sorted({float(x) for x in ha_l if float(x) < price}, reverse=True)

        # RAW OHLC is primary. HA only fills a missing side, then may fill the outer level.
        if raw_res:
            r1 = raw_res[0]
            r1_source = "OHLC"
        elif ha_res:
            r1 = ha_res[0]
            r1_source = "HA_FALLBACK"
        else:
            r1 = None
            r1_source = "NONE"
        ...
        r2_source = "NONE"
        if r1 is not None:
            eps = max(atr_1h * 0.01, 1e-9)
            raw_outer = [x for x in raw_res if x > float(r1) + eps]
            ha_outer = [x for x in ha_res if x > float(r1) + eps]
            candidates = sorted(set(raw_outer + ha_outer))
            if candidates:
                r2 = candidates[0]
                r2_source = "OHLC" if any(abs(r2 - x) <= eps for x in raw_outer) else "HA_FALLBACK"

        s2 = None
        s2_source = "NONE"
        if s1 is not None:
            eps = max(atr_1h * 0.01, 1e-9)
            raw_outer = [x for x in raw_sup if x < float(s1) - eps]
            ha_outer = [x for x in ha_sup if x < float(s1) - eps]
            candidates = sorted(set(raw_outer + ha_outer), reverse=True)
            if candidates:
                s2 = candidates[0]
                s2_source = "OHLC" if any(abs(s2 - x) <= eps for x in raw_outer) else "HA_FALLBACK"
    ...
    def _mcdx_context(self, candles: list, mtf: dict) -> dict:
        ...
        long_score = (
            (30.0 if smart >= 55 else 18.0 if smart >= 50 else 0.0)
            + (25.0 if smart_flow >= 60 else 15.0 if smart_flow >= 52 else 0.0)
            + (20.0 if struct_bias == 1 else 8.0 if struct_bias == 0 else 0.0)
            + (10.0 if htf_bull else 0.0)
        )
        short_score = (
            (30.0 if smart <= 45 else 18.0 if smart <= 50 else 0.0)
            + (25.0 if smart_flow <= 40 else 15.0 if smart_flow <= 48 else 0.0)
            + (20.0 if struct_bias == -1 else 8.0 if struct_bias == 0 else 0.0)
            + (10.0 if htf_bear else 0.0)
        )
        return {
            "long_score": round(long_score, 1),
            "short_score": round(short_score, 1),
            "smart_money": round(smart, 1),
            "smart_flow": round(smart_flow, 1),
            "structure": structure,
            "ready": True,
        }
    ...
    def _entry_triggers_15m(
        ...
        if s1 is not None:
            ...
            if rejection:
                long_trigger = True
                long_name = "1H_S1__15M_REJECTION"
            elif displacement:
                long_trigger = True
                long_name = "1H_S1__15M_DISPLACEMENT"
            elif reclaim:
                long_trigger = True
                long_name = "1H_S1__15M_RECLAIM"
        ...
        if r1 is not None:
            ...
            if rejection:
                short_trigger = True
                short_name = "1H_R1__15M_REJECTION"
            elif displacement:
                short_trigger = True
                short_name = "1H_R1__15M_DISPLACEMENT"
            elif reclaim:
                short_trigger = True
                short_name = "1H_R1__15M_RECLAIM"
    ...
    def _mapped_proximity_atr(self, s1, r1, atr_1h: float) -> tuple[float, Optional[float]]:
        if s1 is None or r1 is None or atr_1h <= 0:
            return self.reversal_proximity_atr, None
        room_atr = (float(r1)-float(s1))/atr_1h
        if room_atr > 3.0:
            return min(room_atr/5.0, 1.0), room_atr
        return self.reversal_proximity_atr, room_atr

    def _open_ended_dynamic_proximity(
        self,
        direction: str,
        long_score: float,
        short_score: float,
        ...
        """Scale OPEN_SKY/FLOOR proximity from 0.60 toward 1.50 ATR using MCDX strength."""
        if direction == "long":
            directional_score = long_score
            gap = long_score-short_score
            score_strength = self._clamp((directional_score-self.min_context_score)/25.0, 0.0, 1.0)
            gap_strength = self._clamp((gap-self.mcdx_dominance_gap)/25.0, 0.0, 1.0)
            flow_strength = self._clamp((smart_flow-self.long_flow_min)/13.0, 0.0, 1.0)
        else:
            directional_score = short_score
            gap = short_score-long_score
            score_strength = self._clamp((directional_score-self.min_context_score)/25.0, 0.0, 1.0)
            gap_strength = self._clamp((gap-self.mcdx_dominance_gap)/25.0, 0.0, 1.0)
            flow_strength = self._clamp((self.short_flow_max-smart_flow)/13.0, 0.0, 1.0)

        strength = 0.40*score_strength + 0.35*gap_strength + 0.25*flow_strength
        proximity = self.open_ended_proximity_atr + strength*(self.open_ended_max_proximity_atr-self.open_ended_proximity_atr)
        proximity = self._clamp(proximity, self.open_ended_proximity_atr, self.open_ended_max_proximity_atr)
    ...
    async def analyze(self, candles: list, current_price: float, mtf_candles: dict = None) -> Signal:
        if len(candles) < 120:
            return Signal(
                ...
                0.0,
                "15M Sentinel warmup",
                metadata={"strategy": "SENTINEL", "version": self.VERSION},
            )

        mtf = mtf_candles or {}
        self._latest_15m = list(candles)
        self._latest_mtf = mtf
        close = float(candles[-1].close)
        sr = self._sr_map_1h(mtf, close)
        meta = {"strategy": "SENTINEL", "version": self.VERSION, "sr_tf": "1H_HYBRID", "entry_tf": "15M_RAW"}
        meta.update({k: v for k, v in sr.items() if k != "ready"})
        if not sr.get("ready"):
        ...
        s1, s2, r1, r2 = sr.get("s1"), sr.get("s2"), sr.get("r1"), sr.get("r2")
        ...
        meta["mcdx_gate"] = {
            "long_pass": mcdx_long_pass,
            "short_pass": mcdx_short_pass,
            "long_score": round(long_score, 1),
            "short_score": round(short_score, 1),
            "long_gap": round(long_gap, 1),
            "short_gap": round(short_gap, 1),
            "min_score_gte": self.min_context_score,
        ...
        open_long_prox, open_long_strength = self._open_ended_dynamic_proximity("long", long_score, short_score, smart_flow)
        open_short_prox, open_short_strength = self._open_ended_dynamic_proximity("short", long_score, short_score, smart_flow)

        long_prox = open_long_prox if sr.get("open_sky_long") else mapped_prox
        short_prox = open_short_prox if sr.get("open_floor_short") else mapped_prox
        long_proximity_mode = (
            "OPEN_SKY_MCDX_DYNAMIC"
            if sr.get("open_sky_long")
            else ("WIDE_ROOM_1_5" if room_for_prox is not None and room_for_prox > 3.0 else "NORMAL_0.30")
        )
        short_proximity_mode = (
            "OPEN_FLOOR_MCDX_DYNAMIC"
            if sr.get("open_floor_short")
            else ("WIDE_ROOM_1_5" if room_for_prox is not None and room_for_prox > 3.0 else "NORMAL_0.30")
        )
        ...
        meta.update({
            ...
            "long_proximity_mode": long_proximity_mode,
            "short_proximity_mode": short_proximity_mode,
            "proximity_room_atr": round(room_for_prox, 2) if room_for_prox is not None else None,
            "open_sky_mcdx_strength": round(open_long_strength, 3),
            "open_floor_mcdx_strength": round(open_short_strength, 3),
        })

        long_context = (
            sx.get("bias") != "BEAR"
            and sx.get("structure") != "BEAR"
            and (sx.get("sme_bull") or sx.get("bias") == "BULL")
            and mcdx_long_pass
        )
        short_context = (
            sx.get("bias") != "BULL"
            and sx.get("structure") != "BULL"
            and (sx.get("sme_bear") or sx.get("bias") == "BEAR")
            and mcdx_short_pass
        )
        ...
        if sr.get("long_map_ready") and trg["long"] and long_context:
            if s2 is not None:
                ...
                long_stop_basis = "BELOW_1H_S2"
            else:
                long_sl = float(s1)-self.missing_outer_sl_atr*atr_1h
                long_stop_basis = f"S1_MINUS_{self.missing_outer_sl_atr:.2f}ATR1H"
            ...
            if long_risk > 0:
                if r1 is None:
                    ...
                    self._entry_sl = long_sl
                    self._open_ended = True
                    meta.update({
                        "entry_location": "1H_S1",
                        "entry_trigger": trg["long_name"],
                        "stop_loss": long_sl,
                        "take_profit": None,
                        "open_ended_tp": True,
                        "tp_basis": "DYNAMIC_R1_OR_STRUCTURE_EXIT",
                        "stop_basis": long_stop_basis,
                        "room_mode": "OPEN_SKY",
                        "entry_proximity_atr": round(long_prox, 2),
                    })
                    return Signal(
                        SignalType.BUY,
                        self.symbol,
                        current_price,
                        0.0,
                        f"SENTINEL LONG {trg['long_name']} | OPEN_SKY prox {long_prox:.2f}ATR MCDXstr={open_long_strength:.0%} | SL {long_stop_basis} | MCDX L={long_score:.1f} S={short_score:.1f} Δ={long_gap:.1f} flow={smart_flow:.1f}",
                        confidence=min(1.0, 0.50+long_score/200.0),
                        metadata=meta,
                    )
                ...
                if location_atr >= self.min_location_atr and long_reward > 0 and long_rr >= self.min_rr:
                    ...
                    meta.update({
                        ...
                        "entry_trigger": trg["long_name"],
                        "stop_loss": long_sl,
                        "take_profit": float(r1),
                        "open_ended_tp": False,
                        "tp_basis": "1H_R1",
                        "stop_basis": long_stop_basis,
                        "rr_ratio": round(long_rr, 2),
                        "room_mode": "FIXED_R1",
                        "entry_proximity_atr": round(long_prox, 2),
                    })
                    return Signal(
                        ...
                        current_price,
                        0.0,
                        f"SENTINEL LONG {trg['long_name']} | room {location_atr:.2f}ATR | prox {long_prox:.2f}ATR | RR {long_rr:.2f} | MCDX L={long_score:.1f} S={short_score:.1f} Δ={long_gap:.1f} flow={smart_flow:.1f}",
                        confidence=min(1.0, 0.50+long_score/200.0),
                        metadata=meta,
                    )
        ...
        if sr.get("short_map_ready") and trg["short"] and short_context:
            if r2 is not None:
                ...
                short_stop_basis = "ABOVE_1H_R2"
            else:
                short_sl = float(r1)+self.missing_outer_sl_atr*atr_1h
                short_stop_basis = f"R1_PLUS_{self.missing_outer_sl_atr:.2f}ATR1H"
            short_risk = short_sl-close
            meta.update({"planned_short_sl": round(short_sl, 8), "short_stop_basis": short_stop_basis})
            if short_risk > 0:
                if s1 is None:
                    self._open_position = "short"
                    self._entry_price = close
                    self._entry_sl = short_sl
                    self._open_ended = True
                    meta.update({
                        "entry_location": "1H_R1",
                        "entry_trigger": trg["short_name"],
                        "stop_loss": short_sl,
                        "take_profit": None,
                        "open_ended_tp": True,
                        "tp_basis": "DYNAMIC_S1_OR_STRUCTURE_EXIT",
                        "stop_basis": short_stop_basis,
                        "room_mode": "OPEN_FLOOR",
                        "entry_proximity_atr": round(short_prox, 2),
                    })
                    return Signal(
                        SignalType.SELL,
                        self.symbol,
                        current_price,
                        0.0,
                        f"SENTINEL SHORT {trg['short_name']} | OPEN_FLOOR prox {short_prox:.2f}ATR MCDXstr={open_short_strength:.0%} | SL {short_stop_basis} | MCDX S={short_score:.1f} L={long_score:.1f} Δ={short_gap:.1f} flow={smart_flow:.1f}",
                        confidence=min(1.0, 0.50+short_score/200.0),
                        metadata=meta,
                    )
                ...
                meta.update({"location_atr": round(location_atr, 2), "short_rr": round(short_rr, 2), "planned_short_tp": float(s1)})
                if location_atr >= self.min_location_atr and short_reward > 0 and short_rr >= self.min_rr:
                    ...
                    meta.update({
                        ...
                        "entry_trigger": trg["short_name"],
                        "stop_loss": short_sl,
                        "take_profit": float(s1),
                        "open_ended_tp": False,
                        "tp_basis": "1H_S1",
                        "stop_basis": short_stop_basis,
                        "rr_ratio": round(short_rr, 2),
                        "room_mode": "FIXED_S1",
                        "entry_proximity_atr": round(short_prox, 2),
                    })
                    return Signal(
                        ...
                        current_price,
                        0.0,
                        f"SENTINEL SHORT {trg['short_name']} | room {location_atr:.2f}ATR | prox {short_prox:.2f}ATR | RR {short_rr:.2f} | MCDX S={short_score:.1f} L={long_score:.1f} Δ={short_gap:.1f} flow={smart_flow:.1f}",
                        confidence=min(1.0, 0.50+short_score/200.0),
                        metadata=meta,
                    )

        reasons = []
        if sr.get("r1_source") == "HA_FALLBACK":
            reasons.append(f"R1 HA_FALLBACK={float(r1):.6f}")
        if sr.get("s1_source") == "HA_FALLBACK":
            reasons.append(f"S1 HA_FALLBACK={float(s1):.6f}")
        if not sr.get("long_map_ready"):
            reasons.append("LONG needs 1H S1 after OHLC+HA")
        elif s2 is None:
            reasons.append(f"LONG S1-only ready; fallback SL=S1-{self.missing_outer_sl_atr:.2f}ATR1H")
        if not sr.get("short_map_ready"):
            reasons.append("SHORT needs 1H R1 after OHLC+HA")
        elif r2 is None:
            reasons.append(f"SHORT R1-only ready; fallback SL=R1+{self.missing_outer_sl_atr:.2f}ATR1H")
        if room_for_prox is not None:
            reasons.append(
                f"PROX {'WIDE_ROOM_1_5' if room_for_prox > 3.0 else 'NORMAL_0.30'}: room={room_for_prox:.2f}ATR -> zone={mapped_prox:.2f}ATR"
            )
        if sr.get("open_sky_long"):
            reasons.append(
                f"LONG OPEN_SKY dynamic zone <= {long_prox:.2f}ATR1H (MCDX strength={open_long_strength:.0%}, cap={self.open_ended_max_proximity_atr:.2f}) from S1"
            )
        if sr.get("open_floor_short"):
            reasons.append(
                f"SHORT OPEN_FLOOR dynamic zone <= {short_prox:.2f}ATR1H (MCDX strength={open_short_strength:.0%}, cap={self.open_ended_max_proximity_atr:.2f}) from R1"
            )
        if not mcdx_long_pass and not mcdx_short_pass:
            reasons.append(
                f"MCDX wait L={long_score:.1f} S={short_score:.1f} ΔL={long_gap:.1f} ΔS={short_gap:.1f} flow={smart_flow:.1f}"
            )
        if not reasons:
            reasons.append("WAIT 15M trigger/context at 1H S1/R1")
    ...
    def tick_open_position(self, current_price: float, position_key: Optional[str] = None):
        ...
        if not self._open_ended:
            return PositionUpdate(action="hold", reason=f"Sentinel {self._open_position.upper()} fixed 1H target active")
        sr = self._latest_sr or {}
        atr_1h = float(sr.get("atr_1h") or 0.0)
        candles = self._latest_15m or []
        if atr_1h <= 0 or len(candles) < 3:
            return PositionUpdate(action="hold", reason="Sentinel open-ended runner: waiting refreshed 1H/15M map")
        structure = self._structure(candles)
        bar = candles[-1]
        prev = candles[-2]
        close = float(bar.close)
        ...
        if self._open_position == "long":
            r1 = sr.get("r1")
            if r1 is None:
                return PositionUpdate(action="hold", reason="LONG OPEN_SKY: no 1H R1 yet — keep runner")
            near_r = high >= float(r1)-proximity
            bearish_reversal = near_r and close < float(r1)-0.05*atr_1h and close < op and close < prev_close
            if bearish_reversal or structure == "BEAR":
                reason = (
                    f"OPEN_SKY exit: 1H R1 formed at {float(r1):.6f} + 15M bearish reversal"
                    if bearish_reversal
                    else f"OPEN_SKY exit: 1H R1 formed at {float(r1):.6f} + 15M structure BEAR"
                )
        ...
        s1 = sr.get("s1")
        if s1 is None:
            return PositionUpdate(action="hold", reason="SHORT OPEN_FLOOR: no 1H S1 yet — keep runner")
        near_s = low <= float(s1)+proximity
        bullish_reversal = near_s and close > float(s1)+0.05*atr_1h and close > op and close > prev_close
        if bullish_reversal or structure == "BULL":
            reason = (
                f"OPEN_FLOOR exit: 1H S1 formed at {float(s1):.6f} + 15M bullish reversal"
                if bullish_reversal
                else f"OPEN_FLOOR exit: 1H S1 formed at {float(s1):.6f} + 15M structure BULL"
            )
            self._reset_position_state()
            return PositionUpdate(action="close", close_pct=1.0, reason=reason)
