...
- Normal mapped entry proximity is 0.30 ATR(1H).
- OPEN_SKY/OPEN_FLOOR entry proximity is up to 0.60 ATR(1H).
...
class SentinelStrategy(BaseStrategy):
    ...
    def _entry_triggers_15m(self,candles:list,s1,r1,atr_1h:float,long_proximity_atr:Optional[float]=None,short_proximity_atr:Optional[float]=None)->dict:
        ...
        long_proximity=max(0.05,long_p)*atr_1h; short_proximity=max(0.05,short_p)*atr_1h; zone=self.entry_zone_atr*atr_1h
    ...
    async def analyze(self,candles:list,current_price:float,mtf_candles:dict=None)->Signal:
        ...
        mtf=mtf_candles or {}; self._latest_15m=list(candles); self._latest_mtf=mtf; close=float(candles[-1].close)
        sr=self._sr_map_1h(mtf,close)
        meta={"strategy":"SENTINEL","version":self.VERSION,"sr_tf":"1H","entry_tf":"15M"}; meta.update({k:v for k,v in sr.items() if k!="ready"})
        if not sr.get("ready"):
            return Signal(SignalType.HOLD,self.symbol,current_price,0.0,sr.get("reason","1H map unavailable"),metadata=meta)

        self._latest_sr=sr; atr_1h=float(sr["atr_1h"]); s1,s2,r1,r2=sr.get("s1"),sr.get("s2"),sr.get("r1"),sr.get("r2")
        sx=self._sentinel_context(candles,mtf); mc=self._mcdx_context(candles,mtf); self._latest_sx=sx; self._latest_mc=mc
        ...
        meta.update({"long_entry_proximity_atr":round(long_prox,2),"short_entry_proximity_atr":round(short_prox,2)})

        long_score=float(mc.get("long_score",0) or 0); short_score=float(mc.get("short_score",0) or 0); smart_flow=float(mc.get("smart_flow",50) or 50)
        long_gap=long_score-short_score; short_gap=short_score-long_score
        ...
        if sr.get("short_map_ready") and trg["short"] and short_context:
            if r2 is not None:
                short_sl=float(r2)+self.sl_buffer_atr*atr_1h; short_stop_basis="ABOVE_1H_R2"
            else:
                short_sl=float(r1)+self.missing_outer_sl_atr*atr_1h; short_stop_basis=f"R1_PLUS_{self.missing_outer_sl_atr:.2f}ATR1H"
            ...
            if short_risk>0:
                if s1 is None:
                    ...
                    meta.update({"entry_location":"1H_R1","entry_trigger":trg["short_name"],"stop_loss":short_sl,"take_profit":None,"open_ended_tp":True,"tp_basis":"DYNAMIC_S1_OR_STRUCTURE_EXIT","stop_basis":short_stop_basis,"room_mode":"OPEN_FLOOR","entry_proximity_atr":round(short_prox,2)})
                    return Signal(SignalType.SELL,self.symbol,current_price,0.0,f"SENTINEL SHORT {trg['short_name']} | OPEN_FLOOR | SL {short_stop_basis} | MCDX S={short_score:.1f} L={long_score:.1f} Δ={short_gap:.1f} flow={smart_flow:.1f}",confidence=min(1.0,0.50+short_score/200.0),metadata=meta)
                location_atr=(float(r1)-float(s1))/atr_1h; short_reward=close-float(s1); short_rr=short_reward/short_risk if short_risk>0 else 0.0
                ...
                if location_atr>=self.min_location_atr and short_reward>0 and short_rr>=self.min_rr:
                    ...
                    meta.update({"entry_location":"1H_R1","entry_trigger":trg["short_name"],"stop_loss":short_sl,"take_profit":float(s1),"open_ended_tp":False,"tp_basis":"1H_S1","stop_basis":short_stop_basis,"rr_ratio":round(short_rr,2),"room_mode":"FIXED_S1","entry_proximity_atr":round(short_prox,2)})
    ...
    def tick_open_position(self,current_price:float,position_key:Optional[str]=None):
        ...
        structure=self._structure(candles); bar=candles[-1]; prev=candles[-2]
        close=float(bar.close); op=float(bar.open); high=float(bar.high); low=float(bar.low); prev_close=float(prev.close); proximity=self.reversal_proximity_atr*atr_1h
