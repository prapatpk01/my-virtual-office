"""Sentinel V10.1 — Bias-Separated Momentum + Location Core.

V10.1 keeps Sentinel V10's indicators, exact setup families, 5M execution,
forecast, fee-aware risk and lifecycle management. The only architectural
change is separating directional bias from exact 15M setup classification.

Priority:
1) Exact V10 setup: PULLBACK / BREAKOUT_RETEST / SWEEP_REVERSAL.
2) If no exact setup exists, a tightly-qualified MOMENTUM_CONTINUATION bias
   may allow 5M to search for a reclaim/micro-breakout trigger.

The fallback is intentionally not a simple high-score bypass. It requires a
clear score edge, 2-of-3 momentum alignment, acceptable S/R location/room,
no strong counter EMA20 slope, a valid 2-of-3 market regime, and no strongly
opposing forecast. Forecast remains advisory for all exact V10 setups.
"""
...
class SentinelV101Strategy(SentinelV10Strategy):
    ...
    # Bias fallback thresholds. Exact V10 setups keep their original thresholds.
    BIAS_MIN_SCORE = 7.0
    BIAS_MIN_EDGE = 1.50
    ...
    def __init__(self, symbol: str, **kwargs):
        ...
        # Keep the V10 position key for router/restart compatibility while the
        # metadata/version identify the running logic as V10.1.
    ...
    def _analysis_15m(self, candles: list) -> dict:
        ...
        # Exact V10 setup always wins. Forecast is still advisory here.
        ...
        if rejects or candidate not in {"long", "short"}:
            # Better diagnostics than V10's selected_score=0/None while keeping
            # the trade direction neutral until the fallback is truly qualified.
            out["reason"] = (
        ...
        # Map fallback quality to inherited strict-mode semantics.
