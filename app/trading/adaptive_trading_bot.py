"""Adaptive Momentum v4.0 — 15M-only trading bot."""
...
SUPPORTED_SCHEMAS={"adaptive-momentum-v3.3-15m","adaptive-momentum-v4.0-15m"}
...
class TradingBot:
    def __init__(self,symbol:str,margin_usdt:float=20.0,leverage:int=20,paper:bool=True,state_file:str="",execution_callback:Optional[Callable]=None,risk_usdt:float=RISK_USDT,**_kwargs):
        ...
        self.counts={k:0 for k in ("scans","entries","cooldown","quality","momentum","location","trigger")}; self._identity(); self.load_state()

    @staticmethod
    def _identity():
        try:
            r=sys.modules.get("run_bot") or sys.modules.get("__main__")
            if r is not None and hasattr(r,"logger"): r.logger=logging.getLogger("adaptive_momentum_v4")
            if r is not None and hasattr(r,"BUILD_ID"): r.BUILD_ID="adaptive-momentum-v4.0-15m-2026-08-17"
    ...
    def _direction(self,i:Dict)->str:
        # Direction is derived entirely from 15M EMA8/13 alignment.
        if i.get("ema_bull"): return "LONG"
        if i.get("ema_bear"): return "SHORT"
        return "NEUTRAL"

    def _debug(self,i:Dict,result:str,reason:str)->str:
        d=self._direction(i); s=self.symbol.split("/")[0]; adx=float(i.get("adx",0)); chop=float(i.get("chop",100)); roc=float(i.get("roc9",0)); hist=float(i.get("macd_hist",0))
        ...
        structure="HL/BOS" if d=="LONG" and i.get("structure_long") else "LH/BOS" if d=="SHORT" and i.get("structure_short") else "-"
        if reason=="COOLDOWN": return f"MOMENTUM V4 · {s} · 15M · ⏳ COOLDOWN {self.cooldown_remaining} bars · RESULT: WAIT"
        parts=[f"MOMENTUM V4 · {s} · 15M · {d}",f"EMA8/13 {'BULL' if d=='LONG' else 'BEAR' if d=='SHORT' else 'FLAT'}"]
        if reason=="QUALITY": parts.append(f"❌ Quality ADX {adx:.1f} rising={'YES' if i.get('adx_rising') else 'NO'} · CHOP {chop:.1f}")
        else:
            parts.append(f"✅ Quality ADX {adx:.1f}↑ · CHOP {chop:.1f}")
            if reason=="MOMENTUM": parts.append(f"❌ Momentum 0/2 · MACD Hist {hist:.4f} · ROC9 {roc:.2f}%")
            else:
                parts.append(f"✅ Momentum {mom}/2{' STRONG' if mom==2 else ''} · MACD Hist {hist:.4f} · ROC9 {roc:.2f}%")
                if reason=="LOCATION": parts.append(f"❌ Location 0/2 · BB/Structure {structure}")
                else:
        ...
        labels={"QUALITY":"WAIT QUALITY","MOMENTUM":"WAIT MOMENTUM","LOCATION":"WAIT LOCATION","TRIGGER":"WAIT EMA CROSS","LONG":"ENTRY LONG","SHORT":"ENTRY SHORT","RISK":"WAIT RISK"}
    ...
    def _build(self,i:Dict,d:str):
        ...
        return {"direction":d,"strategy":"adaptive_momentum_v4_15m","trigger":"EMA8/13 cross","entry":entry,"sl":sl,"tp":tp2,"tp1":tp1,"tp2":tp2,"size":size,"risk_usdt":size*risk,"sl_pct":100*risk/max(entry,1e-12),"ema8":float(i["ema8"]),"ema13":float(i["ema13"]),"macd":float(i["macd"]),"macd_signal":float(i["macd_signal"]),"macd_hist":float(i["macd_hist"]),"bb_mid":float(i["bb_mid"]),"bb_upper":float(i["bb_upper"]),"bb_lower":float(i["bb_lower"]),"atr":atr,"adx":float(i["adx"]),"chop":float(i["chop"]),"roc9":float(i["roc9"])}
    ...
    def _close(self,price:float,reason:str):
        ...
        self.position=None
        if reason in {"EMA_CROSS_BACK","MOMENTUM_LOST"}:self.cooldown_remaining=COOLDOWN_BARS
    ...
    def on_bar(self,i:Dict,_i1=None,_i4=None,price:float=0.0):
        ...
        if i.get("schema") not in SUPPORTED_SCHEMAS: raise RuntimeError(f"MOMENTUM_V4_SCHEMA_MISMATCH: {i.get('schema')}")
        if self.position:
            ...
            # Exit early only if both momentum tools oppose the position.
            if p.direction=="LONG" and int(i.get("momentum_score_long",0))==0:return self._close(px,"MOMENTUM_LOST")
            if p.direction=="SHORT" and int(i.get("momentum_score_short",0))==0:return self._close(px,"MOMENTUM_LOST")
        ...
        d=self._direction(i)
        if d=="NEUTRAL": self.last_signal="MOMENTUM V4 · 15M · NEUTRAL · WAIT EMA8/13"; return None
        # Quality hard gate: trend must have enough strength and not be too choppy.
        ...
        # Momentum: MACD12/26/9 OR ROC9 in the intended direction (1/2 pass).
        ...
        # Location: Bollinger Band position OR market structure (HL/LH/BOS), 1/2 pass.
        ...
        # Execution trigger remains a fresh EMA8/13 cross on the current closed 15M candle.
