...
def _factory(symbols:list,config:dict):
    # V5 is now the authoritative Trend Confirm implementation.  A stale
    # USE_LAYER1_4H=false in Railway previously caused a silent fallback to the
    # legacy strategy, which is why production logs still showed 4/4 + old Q.
    # Keep a separate explicit escape hatch only for rollback/debugging.
    if not _env_bool("TREND_CONFIRM_V5", True):
        logger.warning("[TREND CONFIRM] TREND_CONFIRM_V5=false -> legacy factory")
    ...
    logger.warning("[TREND CONFIRM V5.1] factory active for %d symbols; USE_LAYER1_4H=%s",len(symbols),os.getenv("USE_LAYER1_4H","<unset>"))
...
def _log_scan(self,symbol,strategy_name,price,signal):
    if str(strategy_name).startswith("TrendConfirm("):
        ...
        if str(meta.get("trend_confirm_version","")).startswith("5") or macro.get("layer_role")=="DIRECTION_ONLY":
            ...
            logger.info("[SCAN] %-28s %-22s px=%-11.4f sig=%-5s | L1 4H=%s score=%s/100 (B=%s S=%s) | L2 1H=%s Q=%s/100 [ADX %.1f=%s/25 | CHOP %.1f=%s/20 | STRUCT %s=%s/20 | MOM %s=%s/15 | ROOM %sR=%s/20] hard=%s | 15M=%s | %s",strategy_name,symbol,price,sig_type,macro.get("state","?"),macro.get("score","?"),macro.get("bull_score","?"),macro.get("bear_score","?"),ctx.get("label","?"),ctx.get("score","?"),float(ctx.get("adx",0) or 0),c.get("adx","?"),float(ctx.get("chop",0) or 0),c.get("chop","?"),str(ctx.get("structure","?")).upper(),c.get("structure","?"),"ALIGNED" if ctx.get("momentum_aligned") else "OPPOSED",c.get("momentum","?"),ctx.get("room_r","?"),c.get("room","?"),ctx.get("hard_block","?"),meta.get("entry_trigger_owner") or meta.get("entry_trigger") or meta.get("direction_15m","WAIT"),getattr(signal,"reason","")); return
...
def install()->None:
    ...
    logger.warning("[TREND CONFIRM V5.1] installed: authoritative factory + component log + ADX peak 25-46, decay 46-65, >65=0 + RR 1:2")
install()
