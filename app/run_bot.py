...
def build_config() -> dict:
    return {
        ...
        "candle_tf":    os.environ.get("CANDLE_TF", "1h"),      # 1h | 15m | 4h
...
async def _run_backtest(crypto_bot, config: dict, telegram):
    ...
    backtestable = [s for s in crypto_bot.strategies
                    if isinstance(s, (MACDEMAStrategy, MomentumScoreStrategy,
                                      UTBotStrategy, WTADXStrategy, KNNStrategy))]
    if not backtestable:
        return
...
async def main():
    ...
    logger.info("Stopping all bots — open positions saved, will restore after restart")
