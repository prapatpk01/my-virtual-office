"""Adaptive SMC v15 runner: 15M-only SMC execution, Telegram charts and stats."""
...
BUILD_ID = "adaptive-smc-v15-15m-only-2026-08-05"
logging.basicConfig(
...
logger = logging.getLogger("adaptive_smc_v15")
...
def trade_text(order_type: str, payload: dict, paper: bool) -> str:
    ...
    if order_type.startswith("OPEN_"):
        structure = "CHoCH+BOS" if payload.get("choch_level") and payload.get("bos_level") else "CHoCH" if payload.get("choch_level") else "BOS"
        return (
            f"{'🟢' if direction == 'LONG' else '🔴'} SMC15 {direction} — {symbol}\n"
            f"Mode: {mode}\nTF: 15M ONLY\n"
            f"Trend: EMA20 {'>' if direction == 'LONG' else '<'} EMA50\n"
            f"Liquidity Sweep: ✅\nStructure: {structure}\n"
            f"Zone: {payload.get('zone_type', '-')} [{float(payload.get('zone_low', 0)):,.6f}–{float(payload.get('zone_high', 0)):,.6f}]\n"
...
def stats_text(trades, bots, prices, paper: bool, margin: float) -> str:
    ...
    for symbol, bot in bots.items():
        ...
        pnl = (current - position.entry) * position.size if position.direction == "LONG" else (position.entry - current) * position.size
        floating += pnl
        lines += [
...
def chart(candles, payload: dict, path: str) -> bool:
    ...
    axis.plot(ema20_values, label="EMA20", linewidth=1.2)
    axis.plot(ema50_values, label="EMA50", linewidth=1.2)
    zone_low, zone_high = float(payload.get("zone_low", 0)), float(payload.get("zone_high", 0))
    if zone_low > 0 and zone_high > zone_low:
        axis.axhspan(zone_low, zone_high, alpha=0.18, label=f"{payload.get('zone_type', 'ZONE')}")
    ...
    axis.set_title(f"{payload.get('symbol')} {payload.get('direction')} | SMC v15 | TF15M | {payload.get('trigger')}")
...
async def main() -> None:
    ...
    update_offset = 0
    state_dir = os.getenv("BOT_STATE_DIR", "/tmp/adaptive_smc_v15")
    ledger_path = os.getenv("TRADE_LEDGER_FILE", os.path.join(state_dir, "trade_ledger_v15.json"))
    ...
    def telegram_api(method: str, fields: dict, photo: str = ""):
        ...
        if not photo:
            ...
            boundary = "----SMC15" + uuid.uuid4().hex
            parts = []
            for key, value in fields.items():
                parts += [f"--{boundary}\r\n".encode(), f'Content-Disposition: form-data; name="{key}"\r\n\r\n'.encode(), str(value).encode(), b"\r\n"]
            image = open(photo, "rb").read()
            parts += [f"--{boundary}\r\n".encode(), b'Content-Disposition: form-data; name="photo"; filename="smc15_chart.png"\r\n', b"Content-Type: image/png\r\n\r\n", image, b"\r\n", f"--{boundary}--\r\n".encode()]
    ...
    def execute(order_type: str, payload: dict):
        if order_type.startswith("OPEN_"):
            ...
            if telegram_enabled:
                image_path = f"/tmp/smc_v15_{int(time.time())}_{uuid.uuid4().hex[:6]}.png"
                if not chart(latest_candles.get(str(payload.get("symbol")), []), payload, image_path):
                    raise RuntimeError("Mandatory SMC15 chart failed")
    ...
    logger.info("Adaptive SMC v15 | build=%s | mode=%s | tf=15m-only | telegram=%s", BUILD_ID, "PAPER" if paper else "LIVE", "CONNECTED" if telegram_enabled else "DISABLED")
    if telegram_enabled:
        queue.put_nowait({"kind": "text", "text": f"🤖 Adaptive SMC v15 started\nMode: {'PAPER' if paper else 'LIVE'}\nTF: 15M ONLY\nLogic: EMA20/50 → Sweep → CHoCH or BOS → OB or FVG → Price Action\nSetup expiry: {os.getenv('V15_SETUP_EXPIRY_BARS', '10')} bars\nTP1: {TP1_R:g}R close 50% + BE | TP2: {TP_R:g}R\nMargin: ${margin:.2f} | Risk: ${risk_usdt:.2f}/trade\nCommand: /stats"})
    ...
    try:
            ...
            for symbol in symbols:
                ...
                try:
                    ...
                    if bot.position_open and live_price:
                        ...
                        if event:
                            trades.append({**event, "timestamp": time.time(), "version": "v15"})
                    ...
                    if telegram_enabled:
                        queue.put_nowait({"kind": "text", "text": f"❌ Adaptive SMC v15 error\nSymbol: {symbol}\n{type(error).__name__}: {error}"})
            try:
        ...
        if telegram_enabled:
            queue.put_nowait({"kind": "text", "text": "⏹ Adaptive SMC v15 stopped"})
            try:
