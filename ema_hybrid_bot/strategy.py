"""EMA Hybrid Advanced V2 — balanced frequency with execution confirmation.

Pipeline: H1 direction -> M15 pullback/value -> liquidity/PA -> M5 execution.
Key V2 changes:
- blocks very poor 1H quality instead of letting score bypass Q
- M15 only has to agree with H1 momentum; EMA200 alignment is no longer mandatory
- M5 trigger may occur within a short recent window, increasing usable entries
- liquidity entries require an execution trigger; sweep alone is not enough
- wider EMA/Fibonacci location tolerance to avoid missing valid pullbacks
- keeps Structure + 0.25 ATR SL and 2TP model (1R trim 60%, TP2 liquidity/swing >=1.5R)
"""
...
class EMAHybridProStrategy(base.PrecisionTrendStructureV12):
    ...
    QUALITY_MIN = float(os.getenv("EMA_ADV_QUALITY_MIN", "45"))
    HARD_ADX_MIN = float(os.getenv("EMA_ADV_HARD_ADX_MIN", "12"))
    HARD_CHOP_MAX = float(os.getenv("EMA_ADV_HARD_CHOP_MAX", "65"))
    M5_TRIGGER_LOOKBACK = max(1, int(os.getenv("EMA_ADV_M5_TRIGGER_LOOKBACK", "3")))
    SWING_LOOKBACK = 90
    PIVOT_SPAN = 2
    EMA_TOUCH_ATR = float(os.getenv("EMA_ADV_EMA_TOUCH_ATR", "0.30"))
    FIB_TOL_ATR = float(os.getenv("EMA_ADV_FIB_TOL_ATR", "0.20"))
    NEAR_FIB_ATR = float(os.getenv("EMA_ADV_NEAR_FIB_ATR", "0.50"))
    ...
    def _ema_side(d: pd.DataFrame) -> Optional[Side]:
        if len(d) < 200:
            return None
        r = d.iloc[-1]
        if float(r.ema20) > float(r.ema50) > float(r.ema200): return Side.LONG
        if float(r.ema20) < float(r.ema50) < float(r.ema200): return Side.SHORT
        return None

    @staticmethod
    def _m15_agrees(d: pd.DataFrame, side: Side) -> bool:
        """Faster M15 agreement: momentum/trend structure, not full EMA200 stacking."""
        if len(d) < 60:
            return False
        r = d.iloc[-1]
        if side == Side.LONG:
    ...
    def _trend_label(d: pd.DataFrame) -> str:
        if d.empty: return "DATA_ERROR"
        if len(d) < 200: return f"WARMUP({len(d)}/200)"
    ...
    def _price_action(d, side):
        ...
        if side == Side.LONG:
            ...
            if rc>ro and body/rng>=.60: return "STRONG_BULL_CLOSE"
        else:
            ...
            if rc<ro and body/rng>=.60: return "STRONG_BEAR_CLOSE"
        return "NONE"
    ...
    def _m5_trigger(self, df5, side):
        """Accept a valid execution event from the last N closed M5 bars, not only the latest bar."""
        if len(df5)<30:return "NONE"
        d=df5.copy(); c=d.close.astype(float); d["ema8"]=self._ema(c,8); d["ema13"]=self._ema(c,13)
    ...
    @staticmethod
    def _grade(score): return "A+" if score>=9 else "A" if score>=7 else "B" if score>=5 else "BUILDING"

    def _location_state(self,m15,impulse,touch):
        r=m15.iloc[-1]; atr=max(float(r.atr),1e-12)
        lo=impulse["fib_low"]-self.FIB_TOL_ATR*atr; hi=impulse["fib_high"]+self.FIB_TOL_ATR*atr
        if lo<=float(r.close)<=hi or not(float(r.high)<lo or float(r.low)>hi):return True,"FIB_VALUE"
        distance=min(abs(float(r.close)-lo),abs(float(r.close)-hi))/atr
        if touch!="NONE" and distance<=self.NEAR_FIB_ATR:return True,"EMA_NEAR_FIB"
    ...
    def _score(self,h1,m15,df5,side,location_ok,touch,sweep,pa):
        ...
        structure=self._structure_confirm(m15,side); zone=self._ob_fvg(m15,side); m5=self._m5_trigger(df5,side)
    ...
    def _view(self,df1h,df15,df5):
        ...
        if not self._m15_agrees(m15,side):
            ...
            return HybridView(side,"M15_TREND","M15 momentum opposes H1",score=score,grade=self._grade(score))
        impulse=self._impulse(m15,side)
        if not impulse:return HybridView(side,"FIB","no confirmed impulse swing",score=3,grade="BUILDING")
        touch=self._ema_touch(m15,side); location_ok,location=self._location_state(m15,impulse,touch)
        sweep=self._liquidity_sweep(m15,side); pa=self._price_action(m15,side)
        score,grade,structure,zone,m5,vr=self._score(h1,m15,df5,side,location_ok,touch,sweep,pa)
        common=dict(fib_low=impulse["fib_low"],fib_high=impulse["fib_high"],ema_touch=touch,pa=pa,sweep=sweep,
                    structure=structure,zone=zone,m5_trigger=m5,volume_ratio=vr,score=score,grade=grade,location=location)
        if not location_ok:return HybridView(side,"LOCATION",f"waiting value zone: {location}",**common)
        # V2: a sweep is context, never the execution trigger by itself.
        if sweep!="NONE" and m5!="NONE" and score>=self.MIN_SCORE:
            return HybridView(side,"READY","liquidity + M5 execution passed",entry_path="LIQUIDITY",**common)
        # Alternative path keeps frequency: structure plus M5 and either PA or a strong score.
        confirm_ok=structure!="NONE" and m5!="NONE" and (pa!="NONE" or score>=9)
        if confirm_ok and score>=self.STRONG_CONFIRM_SCORE:
            return HybridView(side,"READY","structure + M5 confirmation passed",entry_path="CONFIRM",**common)
        if m5=="NONE":return HybridView(side,"M5_EXEC",f"setup armed; waiting M5 trigger ({self.M5_TRIGGER_LOOKBACK} bars)",**common)
        return HybridView(side,"SCORE",f"confirmation incomplete score={score}",**common)

    def _liquidity_target(self,m15,side,entry,risk,impulse):
        if risk<=0:return None,0.0
        highs,lows=self._pivot_pairs(m15); candidates=[]
        if side==Side.LONG:
            candidates=sorted(set([v for _,v in highs if v>entry]+[float(impulse["high"])]))
            for target in candidates:
                rr=(target-entry)/risk
                if rr>=self.TP2_MIN_RR:return target,rr
        else:
            candidates=sorted(set([v for _,v in lows if v<entry]+[float(impulse["low"])]),reverse=True)
            for target in candidates:
    ...
    def generate_entry(self,df4h,df1h,df15,df5,has_open_position=False):
        ...
        m15=self._prep(df15); impulse=self._impulse(m15,v.side)
        if not impulse:return None
        r=m15.iloc[-1]; entry=float(r.close); atr=max(float(r.atr),1e-12)
        if v.side==Side.LONG:
            sl=min(float(impulse["low"]),float(m15.low.iloc[-8:].min()))-self.SL_BUFFER_ATR*atr; risk=entry-sl; trend=Trend.BULL
        else:
            sl=max(float(impulse["high"]),float(m15.high.iloc[-8:].max()))+self.SL_BUFFER_ATR*atr; risk=sl-entry; trend=Trend.BEAR
        if risk<=0:return None
        tp2,target_rr=self._liquidity_target(m15,v.side,entry,risk,impulse)
        if tp2 is None:return None
        q=self.quality_state_1h(df1h)
        reason=(f"EMA Hybrid Advanced V2 {v.side.value} | Path={v.entry_path} | Score {v.score}/14 {v.grade} | "
                ...
                f"SL=structure+{self.SL_BUFFER_ATR:.2f}ATR | TP1=1R trim60% + SL BE+0.15R | TP2=liquidity/swing {target_rr:.2f}R")
        trigger=f"EMA_ADV_V2_{v.entry_path}_{v.grade}_{v.m5_trigger}_{v.structure}_{v.pa}"
        room_pct=abs(tp2-entry)/max(entry,1e-12); structure_px=float(impulse["low"] if v.side==Side.LONG else impulse["high"])
        return EntrySignal(v.side,entry,sl,tp2,trend,q.q,q.adx,q.chop,SetupType.PULLBACK,trigger,room_pct,atr,structure_px,reason)

    def entry_status(self,df4h,df1h,df15,df5):
        v=self._view(df1h,df15,df5); d=self._trend_diagnostics(df4h,df1h,df15)
        paper=bool(getattr(self.cfg,"paper",False)); open_=self._entry_schedule_open(); sched="24/7 PAPER(OPEN)" if paper else f"24/5 LIVE({'OPEN' if open_ else 'WEEKEND_CLOSED'})"
        side=v.side.value if v.side else "NONE"; ready=v.stage=="READY" and open_
        return (f"EMA-HYBRID-V2 {'READY' if ready else 'WAIT'} | {side} | 4H={d['4H']} | 1H={d['1H']} | 15M={d['15M']} | "
                ...
                f"ChopMax={self.HARD_CHOP_MAX:.0f} | SLBuf={self.SL_BUFFER_ATR:.2f}ATR | TP2Min={self.TP2_MIN_RR:.1f}R | Reason={v.reason}")
